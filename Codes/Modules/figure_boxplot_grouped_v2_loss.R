...
y_range <- list(
    `GWR` = scale_y_continuous(
        limits = c(mean_data[model=="GWR", max(whisk_up)],
                   mean_data[model=="GWR", min(whisk_down) - 0.27*max(box_length)]),
        trans = "reverse"),
    `BRF` = scale_y_continuous(
        limits = c(mean_data[model=="BRF", max(whisk_up)],
                   mean_data[model=="BRF", min(whisk_down) - 0.27*max(box_length)]),
        trans = "reverse"),
    `CF` = scale_y_continuous(
        limits = c(mean_data[model=="CF", max(whisk_up)],
                   mean_data[model=="CF", min(whisk_down) - 0.27*max(box_length)]),
        trans = "reverse")
    )
...
ggplot(data = gdata, 
    ...
    ylab( bquote("Profit Shortfall ($ "~Ha^{-1}~")") ) +
    xlab("") +
