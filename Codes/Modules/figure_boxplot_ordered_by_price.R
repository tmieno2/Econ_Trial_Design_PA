...
my_width <- 0.45
ggplot(data = gdata, 
       aes(x = reorder_within(design, profit, model, fun = mean), y = profit)) +
    ...
    geom_text(data = mean_data, 
              aes(x = reorder_within(design, profit, model), y = profit_low-1, 
                  label = paste0(round(profit, 2), " (", round(profit_sd, 2),")")),
              position = my_dodge, angle = 0, 
              hjust = 1.1, size = 3 ) +
    coord_flip() +
    facet_wrap(~model, ncol = 1, strip.position = "left", scales = "free") +
    ggtitle( paste0("Prices: ", price_df$pLabel[i])) +
