...
# --------------------------------------------------------------
#  function to compute spatial measure for double-block strip design i 
# --------------------------------------------------------------

fn_spatial_measure_strip2 <- function(i, field){
    
    #' assign experimental input rate (N and Nid)
    N_design <- data.table(
        block_id = rep(1:block_num, each = 12),
        plot_in_block_id = rep(1:12, block_num),
        N = N_levels[rep(c(Strip_ls[[i]], Strip_ls[[i]] %>% rev()), times = block_num)]
    ) %>% 
