...
# N_shp: plot-level N rate polygon shapefile
# Nid: N treatment rate id (1, 2, 3, 4, 5, 6)



fn_spatial_measure <- function(N_shp){
    
    n_trt <- length(unique(N_shp$Nid))
	...
	#===gradation measure===#
	N_shp <- data.frame(N_shp) %>% data.table() %>%
	    ...
	    merge(N_shp, ., by="plot_id")
	GR_row <- mean(N_shp$grad_row, na.rm=T)
	GR_col <- mean(N_shp$grad_col, na.rm=T)
	...
	#===all treatment pairs===#
	P_all <- expand.grid(x1=1:n_trt, x2=1:n_trt) %>% data.table() %>%
	...
	P_row <- data.frame(N_shp) %>% data.table() %>%
		.[order(plot_row_id, plot_col_id),] %>%
		#===by row===#
		.[, N_left:=Nid] %>%
		...
		.[, c("pair_1","pair_2")]
	P_col <- data.frame(N_shp) %>% data.table() %>%
		.[order(plot_row_id, plot_col_id),] %>%
		#===by column===#
		.[, N_up:=Nid] %>%
		...
		.[, c("pair_1","pair_2")]
	P_diag_1 <- data.frame(N_shp) %>% data.table() %>%
		...
		.[, pair_2:=pmax(N_up,N_down)] %>%
		.[, c("pair_1","pair_2")]
	P_diag_2 <- data.frame(N_shp) %>% data.table() %>%
	...
	df <- N_shp@data[, c('plot_id','Nid')]
	df$lon <- coordinates(N_shp)[,1]
	df$lat <- coordinates(N_shp)[,2]
	...
	nb <- poly2nb(N_shp, queen=TRUE)        # neighborhood list
	Wls <- nb2listw(nb, style="B")          # listw

	# global Moran's I
	moran_I <- moran.test(N_shp$Nid, Wls)$estimate[1] %>% unname()
	...
	#'  Return results
	# ---------------------------
	SB_result <- data.frame(MSTmin = MSTmin, 
	                        GR_row = GR_row, 
	                        GR_col = GR_col, 
	                        NB_gini = NB_gini, 
	                        moran_I = moran_I, 
	                        van_Es_var = van_Es_var)
